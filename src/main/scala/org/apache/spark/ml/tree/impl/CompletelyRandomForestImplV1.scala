...
/**
  ...
  */
private [spark] object CompletelyRandomForestImplV1 extends Logging {
  /**
  ...
  def run(
    ...
    while (nodeStack.nonEmpty) {
      ...
      CompletelyRandomForestImplV1.selectNodesToSplit(nodeStack, maxMemoryUsage, metadata, rng)
      // Sanity check (should never occur):
      assert(nodesForGroup.nonEmpty,
        s"CompletelyRandomForestImplV1 selected empty nodesForGroup.  Error for unknown reason.")
      ...
      CompletelyRandomForestImplV1.findRandomSplits(baggedInput, metadata, topNodesForGroup, nodesForGroup,
  ...
  private[tree] def selectNodesToSplit(
    ...
    while (nodeStack.nonEmpty && (memUsage < maxMemoryUsage || memUsage == 0)) {
      ...
      val nodeMemUsage = CompletelyRandomForestImplV1.aggregateSizeForNode(metadata, featureSubset) * 8L
