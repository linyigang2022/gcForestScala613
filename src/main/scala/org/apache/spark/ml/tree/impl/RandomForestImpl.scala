...
import org.apache.spark.ml.core.JNAScala
 ...
 */
private[spark] object RandomForestImpl extends Logging {
  ...
  def run(input: RDD[LabeledPoint],
    ...
    input.sparkContext.conf.registerKryoClasses(Array(classOf[TreeStatsAggregator],
      classOf[ImpurityStats], classOf[LearningNode], classOf[NodeIndexInfo],
      classOf[CategoricalSplit], classOf[ContinuousSplit], classOf[ImpurityCalculator],
      classOf[GiniCalculator], classOf[GiniAggregator]))
  ...
  private[tree] def findBestSplits(input: RDD[BaggedPoint[TreePoint]],
    ...
    logWarning(s"nodeToBestSplits Estimates: %.1f M"
      .format(SizeEstimator.estimate(nodeToBestSplits) / (1024 * 1024.0)))
    ...
    nodesForGroup.foreach { case (treeIndex, nodesForTree) =>
      nodesForTree.foreach { node =>
        ...
        val isLeaf =
          (stats.gain < metadata.minInfoGain) ||
...
//    println(s"$impurity - $leftWeight * $leftImpurity - $rightWeight * $rightImpurity = $gain")
  ...
  private[tree] def getContinuousBestSplit(
                     dtStat: TreeStatsAggregator,
                     gainAndImpurityStats: ImpurityStats,
                     numSplits: Int,
                     featureIndexIdx: Int): (Int, ImpurityStats) = {
    ...
    val rightImpurityCalculator =
      dtStat.impurityAggregator.getCalculator(
        result.view(3 + 2 * statSize, 3 + 3 * statSize).toArray, 0)
    (result(0).toInt, new ImpurityStats(result(1), result(2), parentImpurityCalculator,
      leftImpurityCalculator, rightImpurityCalculator))
  }
  ...
  /**
    ...
    */
  private[tree] def binsToBestSplit(binAggregates: TreeStatsAggregator,
                                    splits: Array[Array[Split]],
  ...
  private[tree] def selectNodesToSplit(
    ...
    while (nodeStack.nonEmpty && !groupDone) {
      ...
      val nodeMemUsage = this.aggregateSizeForNode(metadata, featureSubset) * 8L
